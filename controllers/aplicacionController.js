...
const altaAplicacion = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al crear la aplicación.", error: error.message });
  }
};
...
const updateAplicacion = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al actualizar la aplicación. " + error.message });
  }
};

// Eliminar una aplicación por su ID
const deleteAplicacionFisica = async (req, res) => {
  try {
...
const deleteAplicacionLogica = async (req, res) => {
  try {
    ...
    req.flash("success", "Aplicación de vacuna dada de baja exitosamente.");
    res.redirect("/aplicaciones");
  } catch (error) {
    res.status(500).json({ message: "Error al dar de baja la aplicación." });
  }
};
...
module.exports = {
  ...
  editAplicacion,
  updateAplicacion,
  deleteAplicacionFisica,
  deleteAplicacionLogica,
};
